 ...
 *  This file covers the solution to Day 6 puzzles
 */

namespace Advent_Of_Code
{
    /// <summary>
    /// This class implements all methods that are needed to solve the two puzzles of day 6 of Advent of Code 2020
    /// </summary>
    class CustomCustoms
    {
        public CustomCustoms()
        {
            string[] groupData = splitInputIntoGroups();
            List<int> groupYes = GetGroupAnswers(groupData);
            int sum = 0;
            foreach (int num in groupYes)
            {
                sum += num;
            }
        ...
        private List<int> GetGroupAnswers(string[] groupData)
        {
            List<int> retv = new List<int>();
            List<string> groupStrings = new List<string>();
            for (int i = 0; i < groupData.Length; i++)// (string line in groupData)
            {
                if (!groupData[i].Equals("\r"))
                {
                    groupStrings.Add(groupData[i]);
                }
                if (i == groupData.Length - 1 || groupData[i].Equals("\r"))
                {
                    retv.Add(getGroupNumOfChars(groupStrings));
        ...
        private int getGroupNumOfChars(List<string> answers)
        {
            int retv = 0;
            List<char> firstRow = GetListOfChars(answers[0]);
            List<char> toBeRemoved = new List<char>();
            firstRow.Remove('\r');

            foreach (string answer in answers)
            {
                List<char> additionalRow = GetListOfChars(answer);
                additionalRow.Remove('\r');
                foreach (char c in firstRow)
                {
                    if (!additionalRow.Contains(c))
                    {
                        toBeRemoved.Add(c);
                    }
                }
        ...
        private List<char> GetListOfChars(string str)
        {
        ...
        private string[] splitInputIntoGroups()
        {
        ...
        string input = @"edmzkxfoprcnhijtyvl
adxntojykfcvzermplh
