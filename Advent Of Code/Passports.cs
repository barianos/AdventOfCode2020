    ...
    class Passports
    {
        ...
        public int countObjects()
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        if (areValidNP(tempArray))
                        {
                            if (validateFields(tempArray))
                            {
                                retv++;
                            }
                        }
                    }
                    else if (tempArray.Count == 8)
                    {
                        if (validateFields(tempArray))
                        {
                            retv++;
        ...
        private bool validateFields(List<string> fields)
        {
            ...
            foreach (string field in fields)
            {
                ...
                {
                    case "byr":
                        validity.Add(checkYears(fieldData[1], 1920, 2002));
                        break;
                    case "iyr":
                        validity.Add(checkYears(fieldData[1], 2010, 2020));
                        break;
                    case "eyr":
                        validity.Add(checkYears(fieldData[1], 2020, 2030));
                        break;
                    case "hgt":
                        validity.Add(checkHeight(fieldData[1]));
                        break;
                    case "hcl":
                        validity.Add(checkHairColor(fieldData[1]));
                        break;
                    case "ecl":
                        validity.Add(checkEyeColor(fieldData[1]));
                        break;
                    case "pid":
                        validity.Add(checkPID(fieldData[1]));
                        break;
                    case "cid":
                        validity.Add(true);
                        break;
            ...
            foreach (bool valid in validity)
            {
                if (!valid)
                {
                    return false;
        ...
        private bool areValidNP(List<string> fields)
        {
            bool areValid = true;
            foreach (string field in fields)
            {
        ...
        private bool checkPID(string input)
        {
            bool retv = false;
        ...
        private bool checkEyeColor(string input)
        {
            bool retv = false;
        ...
        private bool checkHairColor(string input)
        {
            bool retv = false;
            char[] split = input.ToCharArray();
            if (split[0].Equals('#'))
            {
                for (int i = 1; i < split.Length; i++)//(char part in split)
                {
                    if (split[i] >= '0' && split[i] <= 'f')
                    {
                        retv = true;
                    }
                    else
        ...
        private bool checkHeight(string input)
        {
            bool retv = false;
            char[] split = input.ToCharArray();
            ...
            if (split.Length == 5)
            {
                if (split[3] == 'c' && split[4] == 'm')
                {
                    string test = split[0].ToString() + split[1].ToString() + split[2].ToString();
                    int intTest = Int32.Parse(test);
                    if (intTest >= 150 && intTest <= 193)
                    {
                        retv = true;
                    }
                }
            ...
            {
                if (split[2] == 'i' && split[3] == 'n')
                {
                    string test = split[0].ToString() + split[1].ToString();
                    int intTest = Int32.Parse(test);
                    if (intTest >= 59 && intTest <= 76)
                    {
                        retv = true;
                    }
                }
        ...
        private bool checkYears(string input, int min, int max)
        {
            bool retv = true;
