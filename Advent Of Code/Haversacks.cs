...
namespace Advent_Of_Code
{
    ...
    {
        string[] data;
        string bagOfInterest;// = "shiny gold";
        public Haversacks()
        {
            bagOfInterest = "shiny gold";
            data = splitInputIntoArray();
            string[,] bagRules = createBagRules(data);
            int answer = findNumberOfContainer(bagOfInterest, bagRules);
            Console.WriteLine("The bag can be contained in : " + answer + " other bags");


        }

        private int findNumberOfContainer(string color, string[,] rules)
        {
            //int retv = 0;
            List<string> tiKans = new List<string>();

            for(int i = 0; i< rules.GetLength(0); i++)
            {
                for (int j=1; j< rules.GetLength(1); j++)
                {
                    if(null == rules[i, j])
                    {
                        break;
                    ...
                    if (rules[i,j].Equals(color))
                    {
                        tiKans.Add(rules[i, 0]);
                    }
                }
            }
            ...
            while (true)
            {
                ...
                for (int i = 0; i < rules.GetLength(0); i++)
                {
                    for (int j = 1; j < rules.GetLength(1); j++)
                    {
                        ...
                        if (tiKans.Contains(rules[i, j]))//.Equals(color))
                        {
                            tiKans.Add(rules[i, 0]);
                        }
                    }
                }
                tiKans = tiKans.Distinct().ToList();
                if(check == tiKans.Count) { break; }
            }

            return tiKans.Count;
        }

        private string[,] createBagRules(string[] data)
        {
            string[,] rules =  initializeZeroArray(data.Length, 10);
            string[] fuckMe;
            for (int i = 0; i < data.Length; i++)// (string rule in data)
            {
                fuckMe = data[i].Split("bags contain ");
                rules[i,0] = (fuckMe[0]);
                string[] contained = Regex.Split(fuckMe[1], @", ");

                for (int j = 0; j < contained.Length; j++)
                {
                    rules[i,j + 1] = contained[j];
                }
            }
        ...
        private string[,] cleanRules(string[,] rules)
        {
            for (int i = 0; i < rules.GetLength(0); i++)
            {
                for (int j=0; j< rules.GetLength(1); j++)
                {
                    if(null != rules[i, j])
                    {
                        rules[i, j] = rules[i,j].Replace("bags", "");
                        rules[i, j] = rules[i, j].Replace("bag", "");
                        rules[i, j] = Regex.Replace(
                            rules[i, j], @"[0-9]+", "");
                        rules[i, j] = rules[i, j].Replace(".", "");
                        rules[i, j] = rules[i, j].Replace("\r", "");
                        rules[i, j] = rules[i, j].Trim();
                    }
                }
            }
        ...
        private string[,] initializeZeroArray(int x, int y)
        {
        ...
        private string[] splitInputIntoArray()
        {
            string[] groups = input.Split("\n");
            return groups;
        }

        private string iinput = @"slight red bags contain 1 bright white bag, 2 muted yellow bags.
...
dotted black bags contain no other bags.";
        ...
        private string input = @"shiny aqua bags contain 1 dark white bag.
