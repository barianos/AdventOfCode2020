 ...
 *  This file covers the solution to Day 2 puzzles
 */
...
{
    /// <summary>
    /// This class implements all methods that are needed to solve the two puzzles of day 2 of Advent of Code 2020
    /// </summary>
    class PasswordDebug
    {
        static int count = 0;
        public int analizePasswords()
        {
            foreach (string pass in passwordsArray)
            {
                if (isPassValid(pass))
                {
                    count++;
        ...
        public bool isPassValid(string password)
        {
            ...
            string[] split = password.Split(new Char[] { ':', '-', ' ' },
                                 StringSplitOptions.RemoveEmptyEntries);

            int min = Int32.Parse(split[0]) - 1;
            int max = Int32.Parse(split[1]) - 1;
            char controlChar = Char.Parse(split[2]);

            char[] passwordArray = split[3].ToCharArray();
            if (Equals(passwordArray[min], controlChar) && Equals(passwordArray[max], controlChar))
            {
                retv = false;
            }
            else if (Equals(passwordArray[min], controlChar))
            {
                retv = true;
            }
            else if (Equals(passwordArray[max], controlChar))
            {
                retv = true;
            }//all other posibilities are false, therefore we dont need to use else clause

            return retv;

        }

        public bool isPassValidFirstCase(string password)
        {
            int smallCount = 0;
            bool retv = false;
            ...
            smallCount = split[3].Split(controlChar).Length - 1;
            if (smallCount <= max && smallCount >= min)
            {
                retv = true;
            }
